...
class ViewAllNotesFragment : Fragment(), MenuProvider, AllNotesAdapter.onItemClick,
    AllNotesAdapter.onItemLongClick {
            ...
            when (menuItem.itemId) {
                ...
                R.id.sync -> {
                    ...
                    if (auth.currentUser == null) {
                        Toast.makeText(
                            requireContext(),
                            "Please login with your gmail first",
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                }
            ...
            override fun onQueryTextSubmit(query: String?): Boolean {
                ...
                if (!query.isNullOrEmpty()) {
                    for (note in oldNotes) {
                        if (note.title.contains(query, ignoreCase = true) || note.content.contains(
    ...
    private fun sendNotification(title: String, text: String) {
        val notification =
            NotificationCompat.Builder(requireContext(), CHANNEL_ID).setSmallIcon(R.drawable.logo)
...
//                currentUserNotesRef = database.reference.child("users").child(auth.currentUser!!.uid).child("notes")
                ...
                if (noteContentByteArray.isNotEmpty()) {
                    noteFileRef.putBytes(noteContentByteArray).addOnSuccessListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val toggle = ActionBarDrawerToggle(
            requireActivity(), binding.drawerLayout, binding.toolbar, R.string.open, R.string.close
        )
    ...
    private fun deleteSelectedNotes() {
        ...
        for (note in selectedNotes) {
            ...
            deleteRef.delete().addOnSuccessListener {
                Toast.makeText(requireContext(), "deleted from cloud also", Toast.LENGTH_SHORT)
                    .show()
